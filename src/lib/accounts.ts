...
// ✅ إصلاح مشكلة IM.from is not a function (Worker & Browser)
// ============================================================
...
// ============================================================
// 🔑 إنشاء محفظة (متوافق مع المتصفح و Worker)
// ============================================================
...
function createWalletDirect(network: string): { address: string; privateKey: string } {
  ...
  if (network === 'solana') {
    try {
      ...
      // إذا كان الخطأ بسبب `Keypair`، نحاول إنشاء محفظة EVM بديلة
      if (error.message?.includes('from') || error.message?.includes('IM')) {
        console.warn('⚠️ فشل إنشاء محفظة Solana، محاولة إنشاء محفظة EVM مؤقتة...');
