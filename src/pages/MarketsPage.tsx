...
import type { DiscoveredToken, PipelineStats, ChainId } from '../types';
...
export function MarketsPage({ onAnalyzeToken }: MarketsPageProps) {
  ...
  const runPipeline = useCallback(async () => {
    ...
    setLoading(false);
  }, [activeNetwork, minLiquidityUsd, minVolume24h, minPriceChange24h]);
                            ...
                            <div className="space-y-3">
                              <div className="flex items-center gap-4 flex-wrap">
                                <div className="flex items-center gap-2">
                                  <span className="text-xs text-slate-500">عنوان العملة:</span>
                                  <span className="text-xs font-mono text-slate-300">{token.tokenAddress.slice(0, 20)}...{token.tokenAddress.slice(-8)}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                ...
                                {token.securityFlags.length > 0 && (
                                  <div className="flex items-center gap-2">
                                    ...
                                    {token.securityFlags.map((flag) => (
                                      <span key={flag} className="text-xs px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-400">{flag}</span>
                                    ))}
                                  </div>
                                )}
                                <a
